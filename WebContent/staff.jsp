...
<%
	...
	ArrayList<Employee> emps = empDao.SelectAll();
%>
<%
							...
							<div class="module">
								...
								<table class="datatable-1 table table-bordered table-striped display" >
									...
									<%for(int i=0;i<emps.size();i++){ 
										Employee tmp=emps.get(i);%>
										<tr>
										  <td><%=tmp.getEmployeeNo() %></td>
										  <td><%=tmp.getEmployeeName() %></td>
										  <td><%=tmp.getSex() %></td>
										  <td><%=tmp.getID() %></td>
										  <td><%=tmp.getTel() %></td>
										  <td><div class="controls">
										  		<a href="modsta.jsp?No=<%=tmp.getEmployeeNo() %>">Modify</a>&nbsp&nbsp&nbsp
										  		<a href="DelSta.jsp?No=<%=tmp.getEmployeeNo() %>">Delete</a>
										  </div></td>
										</tr>
									<%} %>
									</tbody>
