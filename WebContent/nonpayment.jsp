						...
						<div class="content">
							<div class="module">
								...
								<table class="datatable-1 table table-bordered table-striped display" >
									...
									<tbody><%for(int i=0;i<books.size();i++){ 
											Book tmp=books.get(i);
											if(tmp.getPayStatus().equals("no")){%>
										<tr>
										  <td><%=tmp.getBookNo() %></td>
										  <td><%=loadingempno %></td>
										  <td><%=clientDao.selectOne(tmp.getClientNo()).getClientName() %></td>
										  <td><div class="controls input-prepend">
												<input type="text" name="roomcost" value="<%=tmp.getPay() %>" style="width: 80px; " readonly="readonly">
												<span class="add-on">$</span>
										  </div></td>
										  <td><div class="controls">
										  		<a href="billsdetail.jsp?billno=<%=tmp.getBookNo() %>">pay</a>
										  </div></td>
										</tr>
										<%}  } %>
									</tbody>
